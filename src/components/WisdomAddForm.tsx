...
import {Button, Form, Icon, Upload, Select, Input, message} from 'antd';
...
export class WisdomAddForm extends React.Component<IProps & FormComponentProps, any> {
  constructor(props: IProps & FormComponentProps) {
    super(props);
    this.state = {
      labels: props.params.labels || [],
    ...
    if (info.file.status === 'done') {
      if (info.file.response.code === 200) {
        info.fileList[info.fileList.length-1] = Object.assign(info.fileList[info.fileList.length-1],{
          url: info.file.response.result.path
        })
        this.setState({
          fileList: info.fileList
        })
  ...
  public render() {
    ...
    return <Form onSubmit={this.handleSubmit} className="wisdom-add-form">
      <FormItem
        {...formItemLayout}
        label="标题："
      >
      ...
      <FormItem label="文件：" {...formProps} help="">
